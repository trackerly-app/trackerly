...
export const updateCompany = async (req, res, next) => {  
  try {
    ...
    const updateQuery = 'UPDATE companies SET (name, website, notes) = ($1, $2, $3) WHERE id=$4 RETURNING *;';
...
export const getApplications = async (req, res, next) => {
  log.info('[trackCtrl - getApplications] Attempting to get applications from database...');
  // Sanitize information in request body
  try {
    const {
      user_id,
    ...
    log.info('[trackCtrl - getApplication] Applications successfully retreived.');
    // Proceed to next middleware
    return next();
  } catch(err) {
    // Return error information to global error handler
...
export const updateApplication = async (req, res, next) => {
  log.info('[trackCtrl - updateApplication] Attempting to update application in the database...');
  // Sanitize information in request body
  ...
  try {
    const {id} = req.body;
    if(id === undefined)
      throw new Error('no applications to update');
    ...
    const result =
      await db.query(
        applicationQuery,
        [position, salary, status, notes, application_url, id]
      );
    // Assign result from query to res.locals
    res.locals.result = result.rows[0];
    console.log('updated application');
    log.info('[trackCtrl - updateApplication] Application successfully updated in the database.');
    // Proceed to next middleware
    return next();
  } catch(err) {
    // Return error information to global error handler
...
export const deleteApplication = async (req, res, next) => {
  log.info('[trackCtrl - deleteApplication] Attempting to delete application in the database...');
  // Sanitize information in request body
  
  try {
    const {id} = req.body;
    if(id === undefined)
      throw new Error('no applications to delete');
    //query for getting the applicaiton info 
    ...
    console.log('deleted application');
    log.info('[trackCtrl - deletedApplication] Application successfully deleted in the database.');
    // Proceed to next middleware
    return next();
  } catch(err) {
    // Return error information to global error handler
